...
namespace K12.Service.Learning.SLRMultiReport
{
     public partial class SLRClassTotalMulti : BaseForm
     {
          ...
          // 輸出word的樣板
          string _SLRClassTotalMulti_ReportCofig = "K12.Service.Learning.Modules.Config.SLRClassTotalMulti.cs_20141022";
          ...
          private Document SetDocument(ClassTotalObj aClass)
          {
               ...
               Cell count1 = cellSchoolYear;
               Cell count2 = cellSchoolYear;
               for (int x = 1; x < 30; x++)
               {
                    if (count1 != null)
                    {
                         count2 = GetMoveRightCell(count1, 1);
                         if (count2 == null)
                         {
                              Write(count1, "加總");
               ...
               Cell totalcell = (Cell)builder.CurrentParagraph.ParentNode;
               ...
               foreach (StudentTotalObj student in studentObjList)
               {
                    ...
                    for (int x = 1; x < 30; x++)
                    {
                         if (cound != null)
                         {
                              totalcell = GetMoveRightCell(cound, 1);
                              if (totalcell == null)
                              {
                                   Write(cound, total.ToString());
                                   break;
                              }
                              else
                              {
                    ...
                    totalcell = cell = Nextrow.FirstCell; //第一格
               }
